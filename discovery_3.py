...
##########################################################
## this file is used to modify the dictionnary in case of new articles or articles that disappear
## see the easy exemple at the end
## ya surement une solution de stockage meilleure que celle là
###########################################################


fichier="C:\DALET\BERT\\dico_586_articles_and_keywords.pkl"

#dico=joblib.load(fichier)

####################################
# Add an article
####################################
article_id="URL: http://www.nytimes.com/2016/04/15/us/california-surfers-look-to-courts-for-relief-against-bay-boys.html"
texte="""PALOS VERDES ESTATES, Calif. — From high atop the oceanside cliffs, the shimmering blue-green water of Lunada Bay appears to be a surfer’s dream.
...
def add(fichier,article_id,texte):
    dico = joblib.load(fichier)
    ...
    dico[article_id]=(test_bis.article_emb,kw)
    joblib.dump(dico,fichier)
    print("article added")
...
def delete(fichier,article_id):
    dico = joblib.load(fichier)
    dico2={}
    for k,v in dico.items():
        if k!=article_id:
            dico2[k]=v
    joblib.dump(dico2, fichier)
...
dico=joblib.load(fichier)
print("Il y a {} articles dans ce dictionnaire.".format(len(dico)))
add(fichier,article_id,texte)
dico=joblib.load(fichier)
print("Il y a {} articles dans ce dictionnaire.".format(len(dico)))
delete(fichier,article_id)
dico=joblib.load(fichier)
print("Il y a {} articles dans ce dictionnaire.".format(len(dico)))